...
use Reaction\Assets\BootstrapAsset;
...
class Nav extends Widget
{
    ...
    {
        parent::init();
        if ($this->route === null && $this->app->getRoute()->controller !== null) {
            $this->route = $this->app->getRoute()->getRoutePath(true);
        }
        if ($this->params === null) {
            $this->params = $this->app->reqHelper->getQueryParams();
        }
    ...
    public function renderItems()
    {
        ...
        return $this->htmlHlp->tag('ul', implode("\n", $items), $this->options);
    }
    ...
    public function renderItem($item)
    {
        ...
        return $this->htmlHlp->tag('li', $this->htmlHlp->a($label, $url, $linkOptions) . $items, $options);
    }
    ...
    protected function isItemActive($item)
    {
        ...
        if (isset($item['url']) && is_array($item['url']) && isset($item['url'][0])) {
            $route = $item['url'][0];
            if ($route[0] !== '/' && $this->app->getRoute()->controller) {
                $route = $this->app->getRoute()->controller->getUniqueId() . '/' . $route;
            }

...
class Tabs extends Widget
{
    ...
    protected function renderItems()
    {
        ...
        foreach ($this->items as $n => $item) {
            ...
            if (isset($item['items'])) {
                ...
                $header = $this->htmlHlp->a($label, "#", $linkOptions) . "\n"
                ...
                if (isset($item['url'])) {
                    $header = $this->htmlHlp->a($label, $item['url'], $linkOptions);
                } else {
                    if (!isset($linkOptions['data-toggle'])) {
                        $linkOptions['data-toggle'] = 'tab';
                    }
                    $header = $this->htmlHlp->a($label, '#' . $options['id'], $linkOptions);
                }

                if ($this->renderTabContent) {
                    $tag = ArrayHelper::remove($options, 'tag', 'div');
                    $panes[] = $this->htmlHlp->tag($tag, isset($item['content']) ? $item['content'] : '', $options);
                }
            }

            $headers[] = $this->htmlHlp->tag('li', $header, $headerOptions);
        }
    ...
    protected function renderDropdown($itemNumber, &$items, &$panes)
    {
        ...
        foreach ($items as $n => &$item) {
            ...
            $panes[] = $this->htmlHlp->tag('div', $content, $options);

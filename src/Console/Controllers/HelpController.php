...
class HelpController extends Controller
{
     ...
     */
    public function actionIndex(RequestApplicationInterface $app, $command = null)
    {
        if ($command !== null) {
     ...
     * @param RequestApplicationInterface $app
     */
    public function actionList(RequestApplicationInterface $app)
    {
     ...
     */
    public function actionListActionOptions(RequestApplicationInterface $app, $action)
    {
        $result = $this->createController($action);
        ...
        /** @var Controller $controller */
        list($controller, $actionId) = $result;
        if (!$controller->hasAction($actionId)) {
            return;
        }

        foreach ($controller->getActionArgsHelp($actionId) as $argument => $help) {
            $description = str_replace("\n", '', addcslashes($help['comment'], ':')) ?: $argument;
            $this->stdout($argument . ':' . $description . "\n");
        }

        $this->stdout("\n");
        foreach ($controller->getActionOptionsHelp($actionId) as $argument => $help) {
     ...
     * @param string                      $action route to action
     */
    public function actionUsage(RequestApplicationInterface $app, $action)
    {
        $result = $this->createController($action);
        ...
        if ($actionId === $controller->defaultAction) {
            ...
            $this->stdout($scriptName . ' ' . $this->ansiFormat($controller->getUniqueId() . '/' . $actionId, Console::FG_YELLOW));
        }

        foreach ($controller->getActionArgsHelp($actionId) as $name => $arg) {
            if ($arg['required']) {
    ...
    protected function validateControllerClass($controllerClass)
    {
        if (class_exists($controllerClass)) {
            ...
            return !$class->isAbstract() && $class->isSubclassOf('Reaction\Console\Routes\Controller');
        }
    ...
    protected function getSubCommandHelp($controller, $actionId)
    {
        ...
        $args = $controller->getActionArgsHelp($actionId);
        ...
        $options = $controller->getActionOptionsHelp($actionId);
        ksort($options);
    ...
    {
        return basename($this->app->reqHelper->getScriptFile());
    }
    ...
    {
        return "\nThis is Reaction version " . \Reaction::getVersion() . ".\n";
    }

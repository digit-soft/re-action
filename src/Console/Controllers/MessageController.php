 ...
 *    console message/config /path/to/myapp/messages/config.php
 * 2. Edit the created config file, adjusting it for your web application needs.
 * 3. Run the 'message/extract' command, using created config:
 *    console message /path/to/myapp/messages/config.php
 */
class MessageController extends Reaction\Console\Routes\Controller
{
    /**
     ...
     * Defaults to 'Reaction::t'. This is used as a mark to find the messages to be
     * translated. You may use a string for single function name or an array for
     * multiple function names.
     */
    public $translator = 'Reaction::t';
    /**
     ...
     */
    public function actionConfig(RequestApplicationInterface $app, $filePath)
    {
        $filePath = Reaction::getAlias($filePath);
        if (file_exists($filePath)) {
            $confirmPromise = $this->confirm("File '{$filePath}' already exists. Do you wish to overwrite it?");
        } else {
        ...
        return $confirmPromise
            ->then(function() use ($filePath) {
                $array = VarDumper::export($this->getOptionValues($this->getCurrentAction()));
                $content = <<<EOD
<?php
/**
 * Configuration file for 'console {$this->getUniqueId()}/{$this->defaultAction}' command.
 *
 * This file is automatically generated by 'console {$this->getUniqueId()}/{$this->getCurrentAction()}' command.
 * It contains parameters for source code messages extraction.
 * You may modify this file to suit your needs.
 *
 * You can use 'console {$this->getUniqueId()}/{$this->getCurrentAction()}-template' command to create
...
EOD;
    ...
    public function actionConfigTemplate(RequestApplicationInterface $app, $filePath)
    {
        ...
        return $confirmPromise
            ->then(function() use ($filePath) {
                if (!copy(Reaction::getAlias('@reaction/Views/message/config.php'), $filePath)) {
                    return reject(new Exception("Configuration file template was NOT created at '{$filePath}'."));
                }
    ...
    public function actionExtract(RequestApplicationInterface $app, $configFile = null)
    {
        ...
        if (in_array($this->config['format'], ['php', 'po'])) {
            ...
            return $this->saveMessagesToPOT($messages, $this->config['messagePath'], $catalog);
        }
        return resolve(true);
    }
    ...
    protected function saveMessagesToDb($messages, $db, $sourceMessageTable, $messageTable, $removeUnused, $languages, $markUnused)
    {
        ...
        $missingLanguages = [];
        $currentPromises = [
            'messages' => (new Query())->select(['id', 'category', 'message'])->from($sourceMessageTable)->all($db),
            'languages' => (new Query())->select(['language'])->from($messageTable)->groupBy('language')->all($db),
        ];
        ...
        //Get current messages and languages
        return all($currentPromises)
            ->then(function($results) use (&$currentMessages, &$currentLanguages, &$missingLanguages, $languages) {
                if (!empty($results['messages'])) {
                    foreach ($results['messages'] as $row) {
                        $currentMessages[$row['category']][$row['id']] = $row['message'];
                    }
                }
                if (!empty($results['languages'])) {
                    foreach ($results['languages'] as $row) {
                ...
                return true;
            //Extract from $messages array New and Obsolete messages
            })->then(function() use (&$currentMessages, &$newMessages, &$obsoleteMessages, $messages, $removeUnused) {
                foreach ($messages as $category => $msgs) {
                    $msgs = array_unique($msgs);

                    if (isset($currentMessages[$category])) {
                        $newMessages[$category] = array_diff($msgs, $currentMessages[$category]);
                        $obsoleteMessages += array_diff($currentMessages[$category], $msgs);
                    } else {
                        $newMessages[$category] = $msgs;
                    }
                }

                foreach (array_diff(array_keys($currentMessages), array_keys($messages)) as $category) {
                    $obsoleteMessages += $currentMessages[$category];
                }

                if (!$removeUnused) {
                    foreach ($obsoleteMessages as $pk => $msg) {
                        if (mb_substr($msg, 0, 2) === '@@' && mb_substr($msg, -2) === '@@') {
                            unset($obsoleteMessages[$pk]);
                        }
                    }
                }
                $obsoleteMessages = array_keys($obsoleteMessages);
                return [$newMessages, $obsoleteMessages];
            //Insert new messages to DB
            })->then(function() use (&$newMessages, &$savedFlag, &$db, $sourceMessageTable, $messageTable, $languages) {
                $srcInsertPromises = [];
                foreach ($newMessages as $category => $msgs) {
                    foreach ($msgs as $msg) {
                        $savedFlag = true;
                        $srcInsertPromises[] = $db->getSchema()
                            ->insert($sourceMessageTable, ['category' => $category, 'message' => $msg])
                            ->then(function($lastPk) use (&$db, $languages, $messageTable) {
                                $promises = [];
                                foreach ($languages as $language) {
                                    $promises[] = $db->createCommand()
                ...
                return !empty($srcInsertPromises) ? all($srcInsertPromises) : resolve(true);
            //Remove or mark unused messages
            })->then(function() use (&$db, &$savedFlag, &$obsoleteMessages, $removeUnused, $markUnused, $sourceMessageTable) {
                ...
                if ($removeUnused) {
                    return $db->createCommand()
                        ->delete($sourceMessageTable, ['in', 'id', $obsoleteMessages])
                        ->execute()
                        ->then(function() {
                            $this->stdout("deleted.\n");
                            return true;
                    ...
                    return (new Query())
                        ...
                        ->where(['in', 'id', $obsoleteMessages])
                        ->all($db)
                        ->then(function($rows) use (&$db, $sourceMessageTable) {
                            $promises = [];
                            foreach ($rows as $row) {
                                $promises[] = $db->createCommand()->update(
                                    $sourceMessageTable,
                                    ['message' => '@@' . $row['message'] . '@@'],
                                    ['id' => $row['id']]
                                )->execute();
...
EOD;
    ...
    protected function initConfig($configFile)
    {
        $configFileContent = [];
        if ($configFile !== null) {
            $configFile = Reaction::getAlias($configFile);
        ...
        $this->config = array_merge(
            $this->getOptionValues($this->getCurrentAction()),
            $configFileContent,
            $this->getPassedOptionValues()
        );
        $this->config['sourcePath'] = Reaction::getAlias($this->config['sourcePath']);
        $this->config['messagePath'] = Reaction::getAlias($this->config['messagePath']);

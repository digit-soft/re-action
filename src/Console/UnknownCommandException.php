...
use Reaction;
use Reaction\Console\Controllers\HelpController;
...
class UnknownCommandException extends Exception
{
     ...
     * @param RequestApplicationInterface $application the console application instance involved.
    ...
    {
        $help = Reaction::$app->router->createController('help', ['app' => $this->app], false);
        if ($help[0] === null || $this->command === '') {
            return [];
        }
        ...
        foreach ($helpController->getCommands() as $command) {
            $result = Reaction::$app->router->createController($command, ['app' => $this->app], false);
            if ($result[0] === null) {
                continue;
            }
            ...
            /** @var $controller Reaction\Console\Routes\Controller */
    ...
    private function filterBySimilarity($actions, $command)
    {
        ...
        // we assume a typo if the Levenshtein distance is no more than 3, i.e. 3 replacements needed

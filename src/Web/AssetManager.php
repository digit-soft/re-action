...
class AssetManager extends Component
{
    ...
    public function getAssetUrl($bundle, $asset)
    {
        if (($actualAsset = $this->resolveAsset($bundle, $asset)) !== false) {
            if (strncmp($actualAsset, '@web/', 5) === 0) {
                ...
                $basePath = Reaction::$app->getAlias('@webroot');
                $baseUrl = Reaction::$app->getAlias('@web');
            } else {
                $asset = Reaction::$app->getAlias($actualAsset);
    ...
    protected function resolveAsset($bundle, $asset)
    {
        ...
        $n = mb_strlen($asset, Reaction::$app->charset);
        foreach ($this->assetMap as $from => $to) {
            $n2 = mb_strlen($from, Reaction::$app->charset);
    ...
    {
        $path = Reaction::$app->getAlias($path);

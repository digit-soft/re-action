...
class Request extends Reaction\Base\Component implements AppRequestInterface
{
    ...
    use RequestLegacyTrait;
    use RequestServiceLocatorTrait;
...
//    }
     ...
     */
    public function setReactRequest(ServerRequestInterface $reactRequest) {
        $this->_reactRequest = $reactRequest;
    }
    ...
    public function _getHeaders()
    {
        if ($this->_headers === null) {
            ...
            $headers = $this->getReactData('headers');
    ...
    public function getServerParam($name, $defaultValue = null) {
        return Reaction\Helpers\ArrayHelper::getValue($this->_syncData, 'serverParams.' . $name, $defaultValue);
    }
    ...
    {
        $reactPath = $this->getReactData('path');
        $reactQuery = $this->getReactData('query');
    ...
    {
        $cookies = [];
        $cookiesSource = $this->getReactData('cookies');
    ...
    /** @var array Old data before sync */
    protected $_syncData = [];

    /**
     * Sync data with react request
     */
    protected function syncWithReactRequest() {
        $data = $this->extractReactRequestData($this->reactRequest);
        $setters = static::getReactDataSetters();
        foreach ($data as $key => $value) {
            if ((isset($this->_syncData[$key]) && $this->_syncData[$key] === $value) || !isset($setters[$key])) continue;
            $setter = $setters[$key];
    ...
    /**
     * Ser react request headers
     * @param array $headers
     */
    protected function setReactHeaders($headers = []) {
        $this->_headers = null;
    }

    /**
     * Ser react request cookies
     * @param array $cookies
     */
    protected function setReactCookies($cookies = []) {
        $this->_cookies = null;
    }
     ...
     * @return array
     */
    protected function extractReactRequestData(ServerRequestInterface $request) {
        $data = [
        ...
        $data['serverParams'] = Reaction\Helpers\ArrayHelper::merge($serverParams, $data['serverParams']);

        return $data;
    }

    protected static function getReactDataSetters() {
        return [
    ...
    /**
     * Get react php request data by key
     * @param string $key
     * @param mixed  $defaultValue
     * @return mixed
     */
    protected function getReactData($key, $defaultValue = null) {
        return Reaction\Helpers\ArrayHelper::getValue($this->_syncData, $key, $defaultValue);
    }
}

...
class ErrorHandler extends \Reaction\Base\ErrorHandler
{
     ...
     */
    public $errorView = '@reaction/Views/errorHandler/error.php';
    /**
     * @var string the path of the view file for rendering exceptions.
     */
    public $exceptionView = '@reaction/Views/errorHandler/exception.php';
    /**
     * @var string the path of the view file for rendering exceptions and errors call stack element.
     */
    public $callStackItemView = '@reaction/Views/errorHandler/callStackItem.php';
    /**
     * @var string the path of the view file for rendering previous exceptions.
     */
    public $previousExceptionView = '@reaction/Views/errorHandler/previousException.php';
    /**
    ...
    protected function renderException($exception)
    {
        ...
        $useErrorView = $response->format === Response::FORMAT_HTML && (!Reaction::isDebug() || $exception instanceof UserException);

        if ($useErrorView && $this->errorAction !== null) {
            $result = $this->app->resolveAction($this->errorAction, 'GET');
            if ($result instanceof Response) {
                return $result;
            } elseif ($result instanceof ResponseBuilderInterface) {
                $response = $result;
            } else {
            ...
            if ($this->shouldRenderSimpleHtml()) {
                // AJAX request
                $response->setBody('<pre>' . $this->htmlEncode(static::convertExceptionToString($exception)) . '</pre>');
            } else {
                ...
                $response->setBody($this->renderFile($file, [
                    'exception' => $exception,
                ]));
            }
        } elseif ($response->format === Response::FORMAT_RAW) {
            $response->setBody(static::convertExceptionToString($exception));
        } else {
            $response->setBody($this->convertExceptionToArray($exception));
        }
    ...
    {
        if (strncmp($class, 'Reaction\\', 4) !== 0) {
            return null;
        }

        $page = $this->htmlEncode(strtolower(str_replace('\\', '-', $class)));
        $url = "https://github.com/digit-soft/re-action/wiki/doc-1.0/$page.html";
        if ($method) {
            $url .= "#$method()-detail";
        }
    ...
    public function renderFile($_file_, $_params_)
    {
        ...
        if ($this->exception instanceof ErrorException || !$this->app->has('view')) {
            ...
            require Reaction::getAlias($_file_);

            return ob_get_clean();
        }

        return $this->app->view->renderFile($_file_, $_params_, $this);
    }
    ...
    {
        return '<a href="https://github.com/digit-soft/re-action" target="_blank">' . $this->htmlEncode(Reaction::getVersion()) . '</a>';
    }
    ...
    public function getExceptionName($exception)
    {
        if (
            ...
            || $exception instanceof \Reaction\Exceptions\UnknownMethodException
        ) {
    ...
    {
        return Reaction::isTest() || $this->app->reqHelper->getIsAjax();
    }
}

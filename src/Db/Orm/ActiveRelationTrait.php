 ...
 *
 * @method ExtendedPromiseInterface one()
 * @method ExtendedPromiseInterface all()
 * @property ActiveRecordInterface $modelClass
 */
trait ActiveRelationTrait
{
    ...
    public $multiple;
    /**
     * @var ActiveRecordInterface the primary model of a relational query.
    ...
    /**
     * Populate relation for single primary model
     * @param string $name
    ...
    /**
     * Populate relation for multiple primary models
     * @param string                       $name
     * @param ActiveRecordInterface[]      $primaryModels
     * @param ActiveRecordInterface[]|null $viaModels
     * @param ActiveQuery|null             $viaQuery
     * @return ExtendedPromiseInterface with ActiveRecordInterface[]
     */
     ...
     * @deprecated
     */
    public function populateRelationOld($name, &$primaryModels)
    {
        if (!is_array($this->link)) {
            throw new InvalidConfigException('Invalid link: it must be an array of key-value pairs.');
        }
        ...
        if ($this->via instanceof self) {
            ...
            $viaQuery->primaryModel = null;
            $viaModels = $viaQuery->populateRelationOld($viaName, $primaryModels);
            $this->filterByModels($viaModels);
        } else {

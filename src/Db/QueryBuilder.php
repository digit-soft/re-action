...
use Reaction\Db\Conditions\HashCondition;
...
class QueryBuilder extends BaseObject implements QueryBuilderInterface
{
     ...
     * ```php
     * [
     *    Reaction\Db\Expressions\Expression::class => Reaction\Db\Expressions\ExpressionBuilder::class
     * ]
     * ```
    ...
    {
        return [
            'NOT' => 'Reaction\Db\Conditions\NotCondition',
            'AND' => 'Reaction\Db\Conditions\AndCondition',
            'OR' => 'Reaction\Db\Conditions\OrCondition',
            'BETWEEN' => 'Reaction\Db\Conditions\BetweenCondition',
            'NOT BETWEEN' => 'Reaction\Db\Conditions\BetweenCondition',
            'IN' => 'Reaction\Db\Conditions\InCondition',
            'NOT IN' => 'Reaction\Db\Conditions\InCondition',
            'LIKE' => 'Reaction\Db\Conditions\LikeCondition',
            'NOT LIKE' => 'Reaction\Db\Conditions\LikeCondition',
            'OR LIKE' => 'Reaction\Db\Conditions\LikeCondition',
            'OR NOT LIKE' => 'Reaction\Db\Conditions\LikeCondition',
            'EXISTS' => 'Reaction\Db\Conditions\ExistsCondition',
            'NOT EXISTS' => 'Reaction\Db\Conditions\ExistsCondition',
        ];
    }
    ...
    {
        return [
            'Reaction\Db\Query' => 'Reaction\Db\Expressions\QueryExpressionBuilder',
            'Reaction\Db\Expressions\Expression' => 'Reaction\Db\Expressions\ExpressionBuilder',
            'Reaction\Db\Conditions\ConjunctionCondition' => 'Reaction\Db\Conditions\ConjunctionConditionBuilder',
            'Reaction\Db\Conditions\NotCondition' => 'Reaction\Db\Conditions\NotConditionBuilder',
            'Reaction\Db\Conditions\AndCondition' => 'Reaction\Db\Conditions\ConjunctionConditionBuilder',
            'Reaction\Db\Conditions\OrCondition' => 'Reaction\Db\Conditions\ConjunctionConditionBuilder',
            'Reaction\Db\Conditions\BetweenCondition' => 'Reaction\Db\Conditions\BetweenConditionBuilder',
            'Reaction\Db\Conditions\InCondition' => 'Reaction\Db\Conditions\InConditionBuilder',
            'Reaction\Db\Conditions\LikeCondition' => 'Reaction\Db\Conditions\LikeConditionBuilder',
            'Reaction\Db\Conditions\ExistsCondition' => 'Reaction\Db\Conditions\ExistsConditionBuilder',
            'Reaction\Db\Conditions\SimpleCondition' => 'Reaction\Db\Conditions\SimpleConditionBuilder',
            'Reaction\Db\Conditions\HashCondition' => 'Reaction\Db\Conditions\HashConditionBuilder',
            'Reaction\Db\Conditions\BetweenColumnsCondition' => 'Reaction\Db\Conditions\BetweenColumnsConditionBuilder',
        ];
    }
    ...
    {
        return $this->prepareInsertValues($table, $columns, $params)->then(
            function($data) use ($table, &$params) {
                list($names, $placeholders, $values, $params) = $data;
     ...
     * @param array $params the binding parameters that will be generated by this method.
     * They should be bound to the DB command later.
     * @return ExtendedPromiseInterface with array array of column names, placeholders, values and params.
     */
    protected function prepareInsertValues($table, $columns, $params = [])
    {
        $schema = $this->db->getSchema();
        return $schema->getTableSchema($table)->then(
            function($tableSchema) use(&$columns, &$schema, &$params) {
                ...
                if ($columns instanceof Query) {
                    ...
                    foreach ($columns as $name => $value) {
                        ...
                        } elseif ($value instanceof \Reaction\Db\Query) {
     ...
     * @param \Reaction\Db\SchemaInterface $schema Schema object to quote column name.
     * @param array $params the parameters to be bound to the generated SQL statement. These parameters will
     * be included in the result with the additional parameters generated during the query building process.
    ...
    public function batchInsert($table, $columns, $rows, &$params = [])
    {
        ...
        return $schema->getTableSchema($table)->then(
            ...
            function($columnSchemas) use(&$schema, &$columns, &$rows, &$params, $table) {
    ...
    protected function prepareUpsertColumns($table, $insertColumns, $updateColumns, &$constraints = [])
    {
        ...
        return $this->getTableUniqueColumnNames($table, $insertNames, $constraints)->then(
            function($uniqueNames) use($updateColumns, $insertNames) {
            ...
            function() use($insertNames) {
                return [[], $insertNames, $insertNames];
            }
        );
    }
     ...
     * The constraints will be unique by their column names.
     * @return ExtendedPromiseInterface with string[] column list.
     */
    protected function getTableUniqueColumnNames($name, $columns, &$constraints = [])
    {
        $schema = $this->db->getSchema();
        ...
        return $schema->getTablePrimaryKey($name)->then(
            //Primary key
            function($primaryKey) use(&$constraints, &$schema, $name) {
                if ($primaryKey !== null) {
                    $constraints[] = $primaryKey;
                }
                return $schema->getTableIndexes($name);
            },
            function() use(&$schema, $name) {
                return $schema->getTableIndexes($name);
            }
        )->then(
            //Indexes
            function($indexes) use(&$constraints, &$schema, $name) {
                /** @var IndexConstraint[] $indexes */
                foreach ($indexes as $constraint) {
                    if ($constraint->isUnique) {
                        $constraints[] = $constraint;
                    }
                }
                return $schema->getTableUniques($name);
            },
            function() use(&$schema, $name) {
                return $schema->getTableUniques($name);
            }
            ...
            //Uniques
            function($uniques) use (&$constraints, &$schema, $name) {
                /** @var Constraint[] $indexes */
                $constraints = array_merge($constraints, $uniques);
                return $constraints;
            },
            function() use(&$constraints) { return $constraints; }
        )->then(
            //Process constraints
            function() use(&$constraints, &$schema, &$columns) {
                // Remove duplicates
     ...
     * @return ExtendedPromiseInterface with string the UPDATE SQL
     */
    public function update($table, $columns, $condition, &$params)
    {
        return $this->prepareUpdateSets($table, $columns, $params)->then(
            function($sets) use ($table, $condition, &$params) {
                list($lines, $params) = $sets;
     ...
     * @param array $params the binding parameters that will be modified by this method
     * so that they can be bound to the DB command later.
     * @return ExtendedPromiseInterface with array an array `SET` parts for an `UPDATE` SQL statement (the first array element) and params (the second array element).
     */
    protected function prepareUpdateSets($table, $columns, $params = [])
    {
        return $this->db->getSchema()->getTableSchema($table)->then(
            function($tableSchema) {
                /** @var TableSchema $tableSchema */
                $columnSchemas = $tableSchema !== null ? $tableSchema->columns : [];
            ...
            function($columnSchemas) use (&$columns, &$params) {
                /** @var ColumnSchemaInterface[] $columnSchemas */
                $sets = [];
                foreach ($columns as $name => $value) {
    ...
    public function createConditionFromArray($condition)
    {
        if (isset($condition[0])) { // operator format: operator, operand 1, operand 2, ...
            ...
            if (isset($this->conditionClasses[$operator])) {
                ...
                $className = 'Reaction\Db\Conditions\SimpleCondition';
            }

<?php
//TODO: Come back later after UploadedFile implementation
    ...
    protected function validateValue($value)
    {
        ...
        switch ($value->error) {
            ...
            case UPLOAD_ERR_PARTIAL:
                $logger->warning('File was only partially uploaded: ' . $value->name, __METHOD__);
                break;
            case UPLOAD_ERR_NO_TMP_DIR:
                $logger->warning('Missing the temporary folder to store the uploaded file: ' . $value->name, __METHOD__);
                break;
            case UPLOAD_ERR_CANT_WRITE:
                $logger->warning('Failed to write the uploaded file to disk: ' . $value->name, __METHOD__);
                break;
            case UPLOAD_ERR_EXTENSION:
                $logger->warning('File upload was stopped by some PHP extension: ' . $value->name, __METHOD__);
                break;
            default:
                break;
        }
    ...
    public function getClientOptions($model, $attribute)
    {
        ...
        if ($this->minSize !== null) {
            ...
            $options['tooSmall'] = $this->formatMessage($this->tooSmall, [
                'attribute' => $label,
                'limit' => $this->minSize,
                'formattedLimit' => Reaction::$app->formatter->asShortSize($this->minSize),
            ]);
        }
        ...
        if ($this->maxSize !== null) {
            ...
            $options['tooBig'] = $this->formatMessage($this->tooBig, [
                'attribute' => $label,
                'limit' => $this->getSizeLimit(),
                'formattedLimit' => Reaction::$app->formatter->asShortSize($this->getSizeLimit()),
            ]);
        }
